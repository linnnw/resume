<template>
    ...
    <el-form :label-position="labelPosition" :rules="rules" :model="dynamicValidateForm" ref="dynamicValidateForm" label-width="100px" class="demo-dynamic">
      ...
      { type: 'email', message: '请输入正确的邮箱地址', trigger: ['blur', 'change'] }
        ...
        >
            <el-input placeholder="请输入您的邮箱地址" v-model="dynamicValidateForm.email"></el-input>
        </el-form-item>

        <el-form-item label="留言" prop="desc">
            <el-input placeholder="请输入要写给我的内容" :rows="7" type="textarea" v-model="dynamicValidateForm.desc"></el-input>
        </el-form-item>
        ...
        methods: {
            ...
            submitForm() {
                this.$refs['dynamicValidateForm'].validate((valid) => {
                    if (valid) {
                        this.$http.post('email',this.dynamicValidateForm).then(res => {
